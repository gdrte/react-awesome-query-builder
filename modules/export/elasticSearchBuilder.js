...
export const elasticSearchFormat = (item, config) => {
    let meta = {
        usedFields: [],
        errors: []
    };
    
    const logic = elasticSearchFormatItem(item, config, meta);
...
//meta is mutable
const elasticSearchFormatValue = (meta, config, currentValue, valueSrc, valueType, fieldWidgetDefinition, fieldDefinition, operator, operatorDefinition) => {
  ...
  if (valueSrc == 'field') {
      ...
      for (const argKey in funcConfig.args) {
          ...
          const formattedArgVal = elasticSearchFormatValue(meta, config, argValue, argValueSrc, argConfig.type, fieldDef, argConfig, null, null);
    ...
    console.log("inside value section")
...
//meta is mutable
const elasticSearchFormatItem = (item, config, meta) => {
    ...
    if (type === 'group' && children && children.size) {
        const list = children
            .map((currentChild) => elasticSearchFormatItem(currentChild, config, meta))
        ...
        value = value.map((currentValue, ind) => {
            ...
            const fv = elasticSearchFormatValue(meta, config, currentValue, valueSrc, valueType, fieldWidgetDefinition, fieldDefinition, operator, operatorDefinition);
            if (fv !== undefined) {
        ...
        const formattedField = { "field": fieldName };
        ...
        if (typeof operatorDefinition.jsonLogic == 'string')
          formatteOp = operatorDefinition.elasticSearchLogic;
        let fn = typeof operatorDefinition.elasticSearchLogic == 'function' ? operatorDefinition.elasticSearchLogic : null
        if (!fn) {
          const rangeOps = ['<', '<=', '>', '>='];
          fn = (rawfield,field, op, val, opDef, opOpts) => {
            console.log("rawfield " + rawfield);
            console.log("op " + op);
            console.log("cardinality " + cardinality);
            console.log("formatteOp " + formatteOp);
            console.log("isRev " + isRev);
            console.log("formattedField " + JSON.stringify(formattedField));
            ...
            {
              if(op == "not_equal")
                return { "not":{"filter":{[formatteOp]: {[rawfield] : formattedValue}}}};
              else if(op == "less")
                return { [formatteOp]: {[rawfield] :{ "lt": formattedValue}}};
              else if(op == "less_or_equal")
                return { [formatteOp]: {[rawfield] :{ "lte": formattedValue}}};
              else if(op == "greater")
                return { [formatteOp]: {[rawfield] :{ "gt": formattedValue}}};
              else if(op == "greater_or_equal")
                return { [formatteOp]: {[rawfield] :{ "gte": formattedValue}}};
              else if(op == "in")
                return { [formatteOp]: {[rawfield] : formattedValue.toString().split("\n")}};
              else
                return { [formatteOp]: {[rawfield] : formattedValue}};
            }              
            else if (cardinality == 2 && rangeOps.includes(formatteOp))
                return { [formatteOp]: [formattedValue[0], formattedField, formattedValue[1]] };
            else{
              if(op == "between")
                return { [formatteOp]: {[rawfield] :{ "gte": formattedValue[0],"lte": formattedValue[1]}}};
              else
        ...
        if (isRev) {
          ruleQuery = { "must_not": ruleQuery };
          ruleQuery = {"bool":ruleQuery}
        }       
